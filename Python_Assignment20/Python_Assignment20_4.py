# Design an automation script which accepts a directory name,deletes all duplicate files from that directory,and writes the names of deleted files into a log file named Log.txt.The Log.txt file should be created inside the target directory.Additionally, display the total execution time of the script on the console.
...
import hashlib
import time
from HeaderFutter import Header, Footer
...
def DirectoryDuplicate(FileName):
    ...
    fobj = open(LogPath, 'w')
    Header("Delete Duplicates with execution time", fobj)
...
def main():
    if len(sys.argv) == 2:
        if sys.argv[1] in ("--h", "--H"):
            print("This script deletes duplicate files, logs them, and shows execution time.")
